...
class ReadTimeWeek extends StatelessWidget {
    ...
    return Column(
      ...
      children: [
        RichText(
          ...
          TextSpan(
            text:
                "\nRecords for read time that the articles achieved in each week of year ($year)",
            style: TextStyle(
          ...
          child: Builder(builder: (context) {
            ...
            for (var i = 1; i <= currentWeek; i++) {
              chartData["Read Time"]!.add(SplineChartData(
                  i.toDouble(),
                  data["$year"]["weeks"]["$i"]["TOTAL_READTIME"].toDouble(),
                  ((i / currentWeek) * 100).toDouble()));
            }
            return SplineArea(
              chartKey,
              chartData: chartData,
