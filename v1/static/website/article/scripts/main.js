let startTime;
const generateAdSpace= (props)=> {
	props.space.onclick= ()=> {
		window.open(props.adData['redirect']);
	}

	if (props.adData['background_mode'] == 'IMAGE') {
		props.space.style.backgroundImage= `url(/assets/ads/${props.adData['id']})`;
	}
...
const initializeSections= (sections, lang) => {
	...
	previousAction.onclick= () => {
		...
		previousAction.innerHTML= `${lang == 'EN' ? 'Previous: ': 'التالي: '} ${sections[(currentActiveSection - 1)].title[lang]}`
		onArticleChanged(currentArticle);
